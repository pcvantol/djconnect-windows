...
The installer rejects service identities, requires the existing hardened runner virtual account, applies scoped ACLs and registers `\DJConnect\InteractiveGuiSmoke` with `/IT` and limited run level. It creates the one-minute trigger with `schtasks`, then replaces and verifies the action through the native ScheduledTasks API: `cmd.exe /d /c` starts the short command wrapper without relying on `schtasks` command-line action parsing or exceeding its `/TR` length limit. The task is explicitly eligible on battery-reporting hosts, which is required for a Parallels Windows VM; it remains bounded and uses no request means no application launch. Before reporting success it starts the task once and requires a non-secret heartbeat proving an interactive, non-zero session. It stores no password, token or GitHub credential. Keep the smoke user signed in during the GitHub smoke run; otherwise installation and the workflow fail closed.
