...
static void WindowsInteractiveGuiSmokeRelayIsIsolated()
{
    ...
    AssertTrue(installer.Contains("New-ScheduledTaskAction -Execute $env:ComSpec", StringComparison.Ordinal) && installer.Contains("Set-ScheduledTask -TaskName $taskName", StringComparison.Ordinal) && installer.Contains("$registeredAction[0].Execute -ne $env:ComSpec", StringComparison.Ordinal), "scheduled task must use and verify a native cmd.exe action rather than relying on schtasks command-line parsing");
    AssertTrue(installer.Contains("-AllowStartIfOnBatteries", StringComparison.Ordinal) && installer.Contains("-DontStopIfGoingOnBatteries", StringComparison.Ordinal) && installer.Contains("$registeredTask.Settings.DisallowStartIfOnBatteries", StringComparison.Ordinal), "interactive relay must remain eligible on a battery-reporting Windows VM");
