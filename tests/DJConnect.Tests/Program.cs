...
static void WindowsInteractiveGuiSmokeRelayIsIsolated()
{
    ...
    AssertTrue(installer.Contains("$taskCommand = \"$env:ComSpec /d /c", StringComparison.Ordinal) && !installer.Contains("'\"{0}\" /d /c", StringComparison.Ordinal) && !installer.Contains("/TR $launcherPath", StringComparison.Ordinal), "scheduled task must register cmd.exe as an unquoted executable path and pass only the wrapper as a quoted argument");
